...
``` html
<!-- 传递了一个字符串 "1+2" -->
<comp some-prop="1+2"></comp>
```

因为它是一个字面量 prop，它的值是字符串 `"1+2"` 而不是一个数值。如果想传递一个真正的 JavaScript 数值，则需要使用 `v-bind`，从而让它的值被当作 JavaScript 表达式计算：

``` html
<!-- 传递真正的数值 -->
<comp v-bind:some-prop="1+2"></comp>
```
